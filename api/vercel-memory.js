...
// Armazenamento em memória global (persiste entre requisições)
const jobsStorage = new Map();

// Função para salvar job em memória
const saveJob = (jobId, jobData) => {
  try {
    jobsStorage.set(jobId, jobData);
    console.log(`Job ${jobId} salvo em memória`);
  } catch (error) {
    console.error(`Erro ao salvar job ${jobId}:`, error);
  }
};

// Função para carregar job da memória
const loadJob = (jobId) => {
  try {
    const jobData = jobsStorage.get(jobId);
    if (jobData) {
      console.log(`Job ${jobId} carregado da memória`);
      return jobData;
    }
  } catch (error) {
...
// Função para criar vídeo simulado (rápido para teste)
...
const startRenderJob = async (videoData) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Processa imediatamente (para teste)
      setTimeout(async () => {
        try {
...
app.get('/api/jobs', (req, res) => {
  try {
    ...
    // Lista todos os jobs em memória
    for (const [jobId, job] of jobsStorage.entries()) {
      jobList.push({
        id: job.id,
        status: job.status,
        progress: job.progress,
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date().toISOString(),
    service: 'Video Generator API (Memory Storage)',
    environment: process.env.NODE_ENV || 'development',
    message: 'API funcionando! Sistema de polling com armazenamento em memória.',
    template: 'teste.json',
    jobsInMemory: jobsStorage.size,
    capabilities: [
      'Sistema de polling para jobs',
      'Armazenamento em memória compartilhada',
