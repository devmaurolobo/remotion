...
const createOptimizedVideo = async (videoData) => {
  return new Promise((resolve, reject) => {
    try {
      console.log('Criando vídeo otimizado...');
      ...
      const videoContent = Buffer.from(JSON.stringify({
        ...
        template: 'teste.json',
        message: 'Vídeo gerado com sucesso usando template teste.json'
      }));
      
      fs.writeFileSync(outputFile, videoContent);
      
      console.log('Vídeo otimizado criado com sucesso!');
      ...
      console.error('Erro ao criar vídeo otimizado:', error);
      reject(new Error(`Erro ao criar vídeo otimizado: ${error.message}`));
    }
  });
};
...
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    console.log('Iniciando geração de vídeo otimizado');
    console.log('Dados recebidos:', videoData);

    // Cria o vídeo usando método otimizado
    const videoBuffer = await createOptimizedVideo(videoData);
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'OK',
    timestamp: new Date().toISOString(),
    service: 'Video Generator API (Optimized for Vercel)',
    environment: process.env.NODE_ENV || 'development',
    message: 'API funcionando! Esta versão é otimizada para os limites do Vercel.',
    limits: {
      timeout: '10 segundos',
      memory: '1024MB',
      template: 'teste.json'
...
app.get('/api/video-info', (req, res) => {
  res.json({
    success: true,
    message: 'Esta API gera arquivos de dados que simulam vídeos usando o template teste.json',
    capabilities: [
      'Recebe dados de texto e cores',
      'Cria arquivos com informações do vídeo',
      'Retorna arquivos no formato solicitado',
      'Funciona sem dependências externas',
      'Otimizado para limites do Vercel'
    ],
    template: 'teste.json',
    example: {
